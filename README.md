...
| **10** | [`assignment-10/`](assignment-10/README.md) | **Make the Loop Tell the Truth — one training step, instrumented end to end.** One Colab-ready notebook ([open in Colab](https://colab.research.google.com/github/shankarpandala/era-v5/blob/main/assignment-10/training_loop.ipynb), committed fully executed, zero downloads) measures all six Session-10 tasks on a real 493K-param byte-level transformer: every tensor in one step traced with each dimension named (39 tensors + grads + Adam's 2/weight state); one gradient verified by hand to **4.2e-10** relative (toy chain lands on the session's 64 and 16.064 exactly; a directional derivative certifies all 493,568 grads at once; the fp32 probe drowns while fp32 backward stays good — the stick, not the gradient); gradient accumulation **broken on purpose** — average-of-averages reads 3.0000 for a 2.6000 batch (15.4%) and, trained on a corpus where the bug cannot hide, converges to 0.0714 vs 0.0424 across 3 seeds (both optima computed by hand first; the buggy arm believes 92% of documents end after 2 bytes when the truth is 50%, *wins* its own doc-weighted metric, and the equal-count control collapses the gap to 0.00000) — plus a found-in-the-wild ratio bias in the "correct" per-step normalization (analytic p*=0.6681, measured); the grad norm reading **16.3×** its median *before* the update while the dashboard's EMA moved +3.9% (placebo-validated detectors, data-chosen caps — tight vs loose disclosed — realized clip factor ×0.119, the session's own number); MFU computed honestly at **30.5%** of measured attainable GEMM (6N shown to be +0.34% only by an exact-identity cancellation, ±11–28% at other T; distance to 40% = matmuls own 47% of step time, d 64→512 lifts the loop to 57.1%); and **0.1 written out by hand in fp32/bf16/fp8-E4M3** bits via an exact-Fraction encoder matched bit-for-bit to torch (ties-to-even proven on purpose-built probes; fp16's flush-to-zero boundary measured at 2.9e-8; choice: bf16). 16 bytes/weight measured exactly. Five adversarial design reviews ran before a line was written; 64 audit checks re-derive every claim from disk ([`audit.py`](assignment-10/audit.py)), 36 tests exec the notebook's own cells, [grader card](assignment-10/GRADERS.md). |
| **11** | [`assignment-11/`](assignment-11/README.md) | **Tune Both Sides — Adam by hand, schedules that lie at step 200, and a learning rate for a width you cannot afford.** One Colab-ready notebook ([open in Colab](https://colab.research.google.com/github/shankarpandala/era-v5/blob/main/assignment-11/optimizers.ipynb), committed fully executed on this Mac's CPU (float64 checks) and GPU (Apple MPS, every sweep)) measures all six Session-11 tasks: Adam reproduced by hand on five gradients and matched to `torch.optim.Adam` to **3e-16** (fp64) / **6 decimals** (fp32) *non-circularly* (torch never stores m̂/v̂), with L2-in-Adam shown to deliver **2.5× more** decay than AdamW at equal λ; bias correction off: the uncorrected step is exactly r(t) = (1−β₁ᵗ)/√(1−β₂ᵗ) times the corrected one — **6.57× at t = 12, within 10% only after step 1,751** at β₂ = 0.999 (the gap **never closes** in a 600-step run, weight norm doubled) vs 0.45× at t = 1 and **stops mattering at step 50** at β₂ = 0.95 (crossover β₂ = 0.99); the update-to-weight ratio of all 21 tensors decomposed as η · coherence / scale, warmup's kink located at **W** (S_η ≈ 1/W) and shown to be *visible* at width 256 (block-matrix S_ρ/S_η 1.48 / 1.64 / 2.27 across seeds) but *cancelled* at width 128, η = 1e-3 (0.09); cosine vs WSD stopped at 200: **1.5983 vs 1.6666**, keep cosine — the WSD trunk wins only if the horizon grows (by 0.0095 at 400), and the sign flips with peak η; the lr sweep at 256/512/1024 (71 runs, 3 seeds, half-octaves): minima 2⁻⁸·⁸⁰ / 2⁻⁹·⁸⁶ / 2⁻¹⁰·⁴¹, slope **−0.79 ± 0.08** inside the −1…−½ theory bracket, a **pre-registered test at width 2048 (102M params) holds**, and the value for 4,096 is **≈ 2e-4, inside [1.8, 2.3]e-4** (×1.57 band at 95%; the basin asymmetry that would shade it low did not replicate at 2048, and the README says so); and tune-both-sides run *both* naive ways: a tuned challenger vs AdamW at a 3e-4 default "shows" **Lion and SGD beating Adam by 0.2**, while fairly tuned on fresh seeds AdamW wins by 0.073 / 0.113 — with clip 1.0 disclosed as a third party every arm depends on (SGD re-tuned without it: 1.7950 at 2^-4.5 (vs 1.5988 clipped, same seed: the clip is worth +0.196)). Seven adversarial design reviews ran before a line was written and overturned six expectations (README §8); **122 audit checks** re-derive every claim from disk ([`audit.py`](assignment-11/audit.py)), 27 tests exec the notebook's own cells, [grader card](assignment-11/GRADERS.md). |
| **12** | [`assignment-12/`](assignment-12/README.md) | **ZeRO on 32 virtual GPUs.** A standalone, executed CPU/Colab notebook trains a real MLP over 32 threaded virtual ranks with DDP and ZeRO-1/2/3. All stages match the full-global-batch Adam reference to **3.33e-16**; persistent state falls **229 → 118.08 → 62.62 → 7.16 KiB/rank**, while layer gathers limit the tracked peak reduction to **3.16×**. Shows unchanged model FLOPs, 32× fewer Adam entries per rank, ZeRO-3’s 1.5× ring traffic, actual collective traces, and a separate mixed-precision scaling projection. **29 tests**, independent artifact audit, [grader card](assignment-12/GRADERS.md). |
| **13** | [`assignment-13/`](assignment-13/README.md) | **Reversible LLM training — a 20M GPT three ways.** Four executed Colab-ready notebooks train a 20.99M-parameter GPT on TinyStories as a residual baseline at batch 32, as a *reversible* transformer at the same batch (activations reconstructed in backward, not stored: one `autograd.Function` serves RevNet/Reformer `euler` coupling, explicit `midpoint` and `momentum` residuals, gradients equal to stored-activation autograd to 1e-9), and at the largest batch that fits. CPU pilot at 5M tokens/arm (defaults are the assignment's 50M): reversible `momentum` won the four-arm screening; at the same batch it kept **560 vs 2,579 MiB** of activations, peaked at **2.67 vs 4.15 GiB**, ran at **1,345 vs 1,869 tok/s** and finished **0.063 nats ahead** (val 2.6332 vs 2.6964); the freed memory bought a **3.25× batch (312 vs 96)** that, at a fixed 5M-token budget, costs 2.3 nats in 63 steps. Exact activation-vs-depth sweep, denormal-float speed trap, read-only audit, **29 tests**, [grader card](assignment-13/GRADERS.md). |
