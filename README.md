...
| **9** | [`assignment-9/`](assignment-9/README.md) | **The Last Inch — silent failures between the model output and the scalar.** One Colab-ready notebook ([open in Colab](https://colab.research.google.com/github/shankarpandala/era-v5/blob/main/assignment-9/loss_harness.ipynb), committed fully executed, zero installs/downloads) makes the three lines between `model(tokens)` and the loss scalar correct **and observable**: shift verified in printed **strings** (match rate 1.0000), padding masks that change the contributing count 508→186, a packed-document seam masked with per-token receipts (0.15 vs 7.03 nats), untrained perplexity **262.2** vs V=259, tied-vs-untied Δ = exactly V·d = 33,152, and a self-written chunked cross-entropy cutting the loss's peak memory **2,379→181 MiB (13.1×)** at GPT-2 vocab scale. A t+2 second head measures the multi-token-prediction gap as conditional entropy (held-out L1 2.5924 / L2 2.9745, gap opens from zero and never closes). The bug zoo quantifies the course's warning: both classic shift bugs train to *lower* loss (0.0045 / 0.0000 vs 2.3386) while emitting `<bos>` forever — loss and quality anti-correlated. 30 tests exec the notebook's own cells; [`audit.py`](assignment-9/audit.py) re-derives every number from disk and fails if this table drifts; [grader card](assignment-9/GRADERS.md). |
