...
| **7** | [`assignment-7/`](assignment-7/README.md) | **Kronecker Embedding V2 — embeddings that carry mathematical structure.** A deterministic 128-d embedding where `emb("9")+emb("9")` carries **exactly 18** (bit-exact float32) and a log dim makes × additive, while 32 char slots keep spelling invertible. A 2-layer CPU transformer using it beats a learned embedding table 6.9× in-range at 2k pairs and **80×** on number tokens never seen at any training input position; magnitude extrapolation is reported as an honest, probe-localized negative. One command (`python run_demo.py`), 27-run matrix, 46 tests, adversarially reviewed, independent audit, self-contained [`report.html`](assignment-7/submission_artifacts/report.html). |
