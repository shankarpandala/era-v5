...
| Packing correctness | PASS | Packed-batch report — `packing_report.json` |
...
| Crash recovery | PASS | Expected and resumed batch ids — `ledgers/consumption_run_main.jsonl`, `checkpoints/run_main_step24.manifest.json` |
| Replay | PASS | Original and replay hashes — `replay_report.json` |
...
- Packing: 384 samples checked, 0 invariant violations, 3 policies exercised.
...
- Packing utilization: 0.867; loss-bearing fraction 0.955; 29203.6 loss-bearing tokens/sec.
- Loss: 6.6440 (first 10 steps) → 5.6375 (last 10 steps).
- Crash at step 28, resumed from `run_main_step24`; expected next batch `485cb38584049454…` matched: True.
- Replay interval [8, 20]: 12 steps, all hashes matched: True.
