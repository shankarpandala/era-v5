...
**The six tasks, each measured (every number re-derived from disk by `audit.py` — 64
checks, verdict PASS):**

| # | task | the number |
|---|---|---|
| 1 | every tensor shape, one line per dimension | 39 tensors traced through one full step; every grad shape == weight shape; Adam holds 2×493,568 state values; all relations audited |
...
| 5 | my own MFU, honestly | **30.5%** of measured attainable GEMM (528 GFLOP/s; denominator flatters vs theoretical peak — stated); exact FLOPs/token 2,951,424 with profiler cross-check (0.07%); 6N is +0.34% *by an exact-identity cancellation of zero-FLOP params vs the attention term* (+11.5% at T=32, −28.3% at T=512); distance to 40% measured: matmuls own 47% of step time, and d 64→512 lifts the same loop 16.5%→**57.1%** — matrices too small, not mystery overhead; session example reproduced: 648/7,912 = **8.1901%** |
...
python run_demo.py --verify-only   # ~5 s: audit the committed artifacts (64 checks)
