"""Assignment 10 — one command re-derives everything.

    python run_demo.py               # full run: execute the notebook (~4-6 min CPU),
                                     # write it back with outputs, then audit
    python run_demo.py --fast        # ~1-2 min smoke run (A10_FAST=1 budgets)
    python run_demo.py --verify-only # no execution: audit the committed artifacts (~5 s)

Pipeline:

    training_loop.ipynb --nbclient--> executed notebook + submission_artifacts/*
                                           |
...
NOTEBOOK = HERE / "training_loop.ipynb"
...
def main() -> int:
    ...
    ap.add_argument("--fast", action="store_true",
                    help="reduced budgets via A10_FAST=1 (overwrites artifacts)")
    ...
    if args.verify_only:
        log.say("execution evidence: the committed training_loop.ipynb itself — the "
